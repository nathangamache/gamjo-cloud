              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 16 }}>
                {datePhotos.map((p, i) => (
                  <div key={p.id} style={{ aspectRatio: 1, borderRadius: 12, background: p.url ? undefined : `hsl(${(p.id * 67 + i * 30) % 360}, 40%, 55%)`, overflow: 'hidden', position: 'relative', cursor: 'pointer' }} onClick={() => p.url && setViewingPhoto(p)}>
                    {p.url ? <img src={p.thumbnail_url || p.url} alt={p.caption} loading="lazy" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,.7)' }}><Image size={24} /></div>}
            ...
            <div key={date}><div className="label" style={{ color: 'var(--warm)', marginBottom: 6, marginTop: 14 }}>{formatDate(date)}</div>
              {datePhotos.map(p => (
                <div key={p.id} className="photo-item" style={{ cursor: p.url ? 'pointer' : undefined }} onClick={() => p.url && setViewingPhoto(p)}>
                  <div className="photo-thumb" style={{ background: p.url ? undefined : `hsl(${(p.id * 67) % 360}, 40%, 55%)` }}>{p.url ? <img src={p.thumbnail_url || p.url} alt="" loading="lazy" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <Image size={20} />}</div>
      ...
      {viewingPhoto && createPortal(
        <div className="photo-lightbox" onClick={() => setViewingPhoto(null)}>
          <div onClick={e => e.stopPropagation()} style={{ position: 'relative', width: '100%', maxWidth: 800, padding: '0 16px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <button onClick={() => setViewingPhoto(null)} style={{ position: 'fixed', top: 16, right: 16, width: 44, height: 44, borderRadius: '50%', background: 'rgba(255,255,255,.2)', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', zIndex: 310 }}><X size={22} color="#fff" /></button>
