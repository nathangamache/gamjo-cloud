  ...
  const TripCard = ({ trip: t, idx, compact }) => {
    ...
    return (
      <div className="vacation-card" style={{ marginBottom: 12, cursor: 'pointer', borderRadius: 16, overflow: 'hidden' }} onClick={() => handleSelectTrip(t)}>
        <div style={{ position: 'relative', height: compact ? 120 : 160, background: cardBanner ? `url(${cardBanner}) center/cover` : `linear-gradient(135deg, #1d3550, #3a6a8c)` }}>
          {/* Dark gradient overlay for text readability */}
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,.7) 0%, rgba(0,0,0,.3) 50%, rgba(0,0,0,.1) 100%)' }} />
          {/* Bottom: title + meta row */}
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: compact ? '10px 14px' : '14px 16px' }}>
            <div style={{ fontWeight: 600, fontSize: compact ? 16 : 18, color: '#fff', lineHeight: 1.2, textShadow: '0 1px 4px rgba(0,0,0,.4)' }}>{t.name}</div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 }}>
              <span style={{ fontSize: compact ? 12 : 13, color: 'rgba(255,255,255,.75)' }}>{t.location} &middot; {formatDateRange(t.start_date, t.end_date)} &middot; {t.member_count || 0} travelers</span>
              {days !== null && days > 0 && days <= 7 && <span style={{ padding: '4px 12px', borderRadius: 12, background: 'rgba(0,0,0,.55)', color: '#fff', fontSize: 13, fontWeight: 600, flexShrink: 0 }}>{days} days!</span>}
              {days !== null && days > 7 && <span style={{ padding: '4px 12px', borderRadius: 12, background: 'rgba(0,0,0,.45)', color: 'rgba(255,255,255,.9)', fontSize: 13, fontWeight: 500, flexShrink: 0 }}>{days} days away</span>}
              {days !== null && days === 0 && <span style={{ padding: '4px 12px', borderRadius: 12, background: 'rgba(0,0,0,.55)', color: '#fff', fontSize: 13, fontWeight: 600, flexShrink: 0 }}>Today!</span>}
            </div>
          </div>
        </div>
      </div>
